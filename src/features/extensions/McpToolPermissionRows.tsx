...
/** A server's on/off at a scope. One Claude Code itself keeps off can't be switched on. */
export function ServerScopeToggle({ server, target, busy }: { server: McpServerLive; target: PermissionTarget; busy?: boolean }) {
  const st = serverAt(target.cascade, target.scope, server.name, target.external ?? []);
  const on = st.on;
  const title =
    target.readOnlyReason ??
    (st.own === null && st.from && st.from !== "claude"
  ...
  return (
    <Toggle
      checked={on}
      disabled={!!busy || target.pending || st.locked || target.readOnlyReason != null}
      onChange={(next) =>
...
export function McpPermissionSummary({ server, target }: { server: McpServerLive; target: PermissionTarget }) {
  const s = serverSummary(target.cascade, target.scope, server.name, toolsOf(server), target.external ?? []);
...
export function McpToolPermissions({ server, target }: { server: McpServerLive; target: PermissionTarget }) {
  ...
  if (target.loadError) {
    return <div className={styles.error}>Unable to read the permission defaults: {target.loadError}</div>;
  }
