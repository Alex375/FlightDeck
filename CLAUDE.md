...
⚠️ Depuis juillet 2026, le **nom AFFICHÉ de l'app est « Flight Deck »** (productName Tauri + titre de fenêtre, menu app macOS, wordmark UI `TosseMark`, 3 messages TCC `Info.plist`, dialogues Réglages/MAJ/notifs, `releaseName`), avec un **nouveau logo** « avion cyan + souffle réacteur corail » (`public/tosse.svg`, `src/ui/TosseMark.tsx`, `src-tauri/app-icon.svg` + icônes régénérées via `pnpm tauri icon`). **C'est un rebrand DISPLAY-ONLY** : l'**identité technique reste `tosse-code`** et NE DOIT PAS changer — identifiant `com.tosse.desktop`, crate/package `tosse-code`, repo GitHub `Alex375/FlightDeck` (⚠️ **le dépôt GitHub est la SEULE pièce d'identité qui a suivi le rebrand** : renommé depuis `Alex375/tosse-code`. L'endpoint updater de `tauri.conf.json` et l'url du dépôt CRM pointent déjà dessus, et `release.yml` passe par `$GITHUB_REPOSITORY` — rien d'autre à changer. Un clone dont l'origin nomme encore `tosse-code` pousse toujours par redirection GitHub, mais ne s'associe PLUS au dépôt CRM via `normalize_remote_url` : son origin doit être repointé sur la nouvelle url), certificat « Tosse Code Self-Signed », projet CRM « Tosse Code », et les filenames internes `public/tosse.svg` + composant `TosseMark`. **NE PAS « corriger » la divergence productName ↔ identifiant/crate** : elle est VOLONTAIRE (changer l'identifiant = nouveau dossier `~/Library/Application Support/…` = perte des conversations + re-grant TCC global). La **vue de gestion d'agents garde aussi le nom « Flight Deck »** (l'app == sa vue phare). Piège updater : les installs déjà en place restent le fichier `Tosse Code.app` (remplacement en place par l'updater) tout en affichant « Flight Deck » ; un nouvel install `.dmg` = `Flight Deck.app`.
...
- **Effort** : la famille **gpt-5.6** (`gpt-5.6-sol/terra/luna`, ids VÉRIFIÉS réels via `model/list`) ajoute les crans `max` + `ultra` (data-driven depuis `supportedReasoningEfforts`). `ultra` ≠ `ultracode` (tier Claude) — mais l'Ultra Codex RÉUTILISE l'animation SLIDER de l'ultracode (remplissage multicolore + pulse + glow, flag `ultraFx` dans `EffortGauge`), SANS le blast plein écran (réservé à ultracode). **Defaults Codex** : modèle `gpt-6-astra` (`models.ts::DEFAULT_CODEX_MODEL`, = `isDefault` de `model/list` depuis codex 0.153), effort `xhigh` (« Extra »), preset sécurité `auto` (`codexControls.ts`). ⚠️ Un défaut n'est tenu que si le binaire INSTALLÉ le propose : `modelPrefs.ts::effectiveDefaultModel` démarre une nouvelle conv Codex sur le défaut stocké s'il figure dans la liste live, sinon sur l'`isDefault` du binaire (le choix stocké est conservé et regagne dès que le binaire le connaît).
...
- `src-tauri/` (Rust) — `supervisor/`, `git/`, `fs/`, `usage/`, `terminal/`, `power/`, `cli_update/`, `tosse/`, `appmcp/`, `voice/`, `wake/`, `store/`, `ipc/`
  ...
  - `git/` : `git/mod.rs` = SEUL service git, enveloppe le **binaire `git` CLI** (pas git2). Porte `remote_url` (⚠️ `remote get-url`, PAS `config --get` qui lit aussi le global → faux « pas d'origin » ; codes 2=pas de remote / 128=pas un dépôt, `LC_ALL=C`), `normalize_remote_url` (LE seul juge que deux URLs = même dépôt ; **jamais par NOM**), `redact_remote_url` (⚠️ expurge l'userinfo d'une url `scheme://user:token@host/…` **au point de CAPTURE**, avant toute persistance ou tout affichage : un clone peut porter un PAT dans son remote, et il finissait en clair dans SQLite et sur la carte TOSSE. Matching intact — `normalize_remote_url` supprimait déjà ce segment. La forme scp `git@host:path` n'est PAS touchée : c'est un utilisateur SSH, pas un secret), `scan_repos` (lit `.git/config`, budget TEMPS 3 s, symlinks non suivis, `truncated`/`unreadable` pour qu'un « rien trouvé » ne passe pas pour un verdict).
...
- **Liste de modèles Claude STATIQUE — décision produit** (`CLAUDE_MODELS` curé). ⚠️ NE PAS rebrancher `list_models` sur le picker ; n'en garder que `supported_effort_levels` + `resolved_model` (suffixe `[1m]` = seul signal wire de la fenêtre 1M). **Usine = le plus récent** : `FACTORY_HIDDEN_MODELS` est DÉRIVÉ — Claude : tout sauf la 1re ligne de chaque `family` (catalogue trié du plus récent au plus ancien), **Mythos jamais affiché d'office** ; Codex (`codexFactoryHidden`) : le plus récent de chaque ligne (Astra/Sol/Terra/Luna/Mini), un modèle sans ligne (`gpt-5.5`) part en Available dès qu'une génération plus récente existe, appliqué à la liste live `model/list`. Défaut Claude = dernier Opus (alias `opus`). Les prefs (`tosse:models`) portent `seen` (valeur → modèle réellement lancé) : un modèle jamais vu — ou un alias qui a changé de cible — reçoit le traitement d'usine, les choix de l'utilisateur restent. ⚠️ **Nouveau modèle Claude = procédure** : l'alias du CLI bouge seul (la ligne alias lance alors le nouveau modèle sous l'ancien label — Opus 5.5 affiché « Opus 5 ») → relabel la ligne alias + bump son `modelId`, ajouter une ligne pinnée (id complet) pour l'ancien modèle, déplacer le bras d'alias de `assembler.rs::model_label`. Les prefs se réconcilient seules.
...
- **Réglages** : modale à rail d'onglets (**sous-onglets + recherche**), briques `SettingsKit.tsx` (`SettingsGroup`/`ToggleRow` avec `disabled`/`disabledReason`, `VersionStatus`, `SubTabs`). Onglet Updates = 2 updaters (app + CLI `claude`) qui ne s'empilent jamais (`isUpdateBannerVisible`). **Sous-onglets** (`SubTabs`) : état mémorisé PAR section dans `settingsUi.subs` (survit à un aller-retour) — General (Display/Durations/System), Conversation (Markdown/Models/Composer), MCP Control (In-app agents/Voice/Remote/Bridge), Notifications (Channels/Fleet/Background). **Recherche** : `settingsSearch.ts` = index plat `SETTINGS_INDEX` + scorer PUR `searchSettings` (mots-clés FR/EN, matching accent-insensible) ; `revealSetting` (`settingsUi`) fait clignoter la ligne trouvée (matching par TITRE visible de la ligne). **Regroupements** (rail passé de 12 à 10 onglets) : Conversation absorbe les ex-onglets **Models** et **Composer** en sous-onglets ; Notifications absorbe l'ex-**General → Alerts** (fleet banner + background-task alert) à côté des canaux OS ; **General → Display** éclaté de sa carte unique de 11 toggles en 3 cartes **Appearance / Thread / Motion**. **Pattern `embedded`** : les sections fusionnées (`ConversationSection`/`ModelsSection`/`ComposerSection`/`NotificationsSection`) prennent une prop `embedded` qui masque leur propre `PageHead` (l'onglet porte le titre, même forme que les sous-groupes de MCP Control) ; elles restent utilisables seules. Deux autres onglets ajoutés sur `dev` : **Behavior** (output style + « bypass permissions », déplacé depuis General) et **Claude Code** (routing sous-agents / dépenses / instructions, **gated** sur un compte Claude connecté via `needsClaude` + latch `claudeSeen`). ⚠️ **INVARIANT** : `SETTINGS_SUBS` (dans `settingsSearch.ts`) doit refléter EXACTEMENT les sous-onglets réellement rendus par le panneau, et **toute entrée d'index d'une section à sous-onglets DOIT porter un champ `sub`** — sinon un résultat de recherche atterrit sur une page vide. Deux tests unitaires (`settingsSearch.test.ts`) gardent ça.
...
- **Où travaille un projet** : clé = le PROJET (table `tosse_project_repos`, migration **v8**, `ON DELETE CASCADE`). Rien ne résout → `scan_local_git_repos` propose les clones du Mac (⚠️ dossiers LOCAUX seulement : filtrer `machine_id.is_none()`, sinon on prend pour racine de scan un `/home/…` qui vit sur une AUTRE machine — message TCC trompeur, et `/home` étant un point de montage autofs, le budget temps du scan peut y passer en entier). Migrations **v6** `repos.tosse_repository_id` (pin), **v7** `conversations.tosse_task_id/_title/_status`, **v15** `repos.remote_origin_url` + `remote_origin_probed_at`, **v16** `repos.remote_origin_note`. **Le schéma est à v16** — cette liste n'est pas un total. ⚠️ **Aucune n'est une FK** (ids du CRM ; une entité supprimée serveur doit DÉGRADER, pas corrompre la ligne). ⚠️ `tosse_repository_id` écrit UNIQUEMENT par `set_repo_tosse_link` et HORS de `RepoRecord` (sinon `upsert_repo`/ses appelants l'effacent).
- **Origin d'un dossier DISTANT (sweep SSH)** : `tosse_probe_remote_origins` lit l'`origin` des dossiers qui vivent sur un serveur appairé et le cache en SQLite (le `git` de ce Mac ne peut pas lire un chemin de là-bas). ⚠️ Il renvoie **`RemoteOriginSweep{changed, skipped, writeErrors}`, JAMAIS un `bool`** : un booléen confondait « rien n'a bougé » avec « je n'ai pas regardé » (refus de concurrence) et n'avait nulle part où porter un échec d'écriture SQLite — deux erreurs silencieuses distinctes. ⚠️ **Les réponses FERMES qui ne sont pas une url (`no-remote` / `not-a-repository` / `gone` / `no-git`) se PERSISTENT** (`remote_origin_note`, `probed_at` estampillé, url cachée INTACTE — un dossier démonté garde l'origin qu'il avait) : les jeter laissait `probed_at` NULL, c.-à-d. « on n'a pas pu demander », et la carte accusait alors la joignabilité du serveur pour un dossier sur lequel il venait de répondre, avec « try again once the server is reachable » pour seul conseil — un remède qui ne pouvait pas marcher. ⚠️ Côté front, le refetch écoute **`dataUpdatedAt` (un sweep qui SE TERMINE), pas la VALEUR de `changed`** : deux sweeps renvoyant `true` d'affilée ne déclenchaient rien, donc Refresh ne faisait visiblement rien dans la situation même où on l'utilise (serveur éteint au lancement, rallumé ensuite). Effet dédoublonné par un curseur module-level — il vit dans un hook que N badges appellent, donc un sweep coûtait N appels `tosse_repo_links`.
